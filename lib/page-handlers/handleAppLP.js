...
export async function handleAppLP(params, mappings, pixels) {
  ...
  const brandConfig = appConfig[mapping.applander || 'cash750'];
  
  // Add base path for assets
  const assetBasePath = '/public/assets';
  
  // Replace variables in the HTML template
  let html = injectHelpers(appLander, tt_pixel_name, fbpixel);
