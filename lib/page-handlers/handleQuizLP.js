...
export async function handleQuizLP(params, mappings, pixels) {
  ...
  const skipDeals = dynamicLanders[mapping.dynlander].skip_deals;

  const html = injectHelpers(quiz, tt_pixel_name, fbpixel)
              ...
              .replace(/{{deals_visibility}}/g, skipDeals ? 'none' : '');
